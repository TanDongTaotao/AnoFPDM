...
This version incorporates edge regularization to improve boundary alignment.
"""
...
def main():
    ...
    if args.num_batches_val != 0:
        ...
        thr_01, diff_min, diff_max, n_min = obtain_hyperpara(
            data_val, diffusion, model, args, dist_util.dev(), edge_loss=args.edge_loss, edge_weight=args.edge_weight
        )
    ...
    while k < args.num_batches:
        ...
        for t in range(len(xstarts['xstart'])):
            ...
            # Flatten the edge maps for cosine similarity calculation
        ...
        logger.log(f"Average Edge Cosine Similarity Loss: {avg_edge_loss:.4f}")
        # --- End of Edge Regularization --- #
        ...
        for n, ratio in enumerate(args.t_e_ratio):
            ...
            if args.save_data:
                ...
                all_edge_losses.append(avg_edge_loss)

                for key in all_terms.keys():
                    gathered_terms = [
                        torch.zeros_like(xstarts[key][0]) for _ in range(dist.get_world_size()) # Gather first timestep
                    ]
                    dist.all_gather(gathered_terms, xstarts[key][0])
                ...
                all_edge_losses_path = os.path.join(image_subfolder, f"edge_loss_{k}.npy")
                np.save(all_edge_losses_path, np.array(all_edge_losses))
