...
def main():
    ...
    while k < args.num_batches:
        ...
        if _safe_get_rank() == 0:
            ...
            logger.log(
                f"pred_pixels_postcc[min={float(post_pixels.min()):.0f}, mean={float(post_pixels.float().mean()):.1f}, max={float(post_pixels.max()):.0f}] cc_filter={args.cc_filter} cc_thr={args.cc_thr}"
            )
            logger.log(
