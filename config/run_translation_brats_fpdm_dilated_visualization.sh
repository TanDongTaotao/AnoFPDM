...
# 设置每个批次可视化的样本数量（建议设置为较小值以避免生成过多图像）
...
do
    ...
    do
        # 设置日志输出目录
        export OPENAI_LOGDIR="${LOG_ROOT}/logs_brats_aba/translation_fpdm_dilated_visualization_${w}_${model_num}_${forward_steps}_${round}_x1"
        # 确保目录存在
        mkdir -p $OPENAI_LOGDIR
        echo "日志将保存在: $OPENAI_LOGDIR"

        # 设置可视化输出目录
        vis_output_dir="${visualization_output_dir}/visualization_${w}_${model_num}_${forward_steps}_${round}"
        ...
        torchrun --nproc-per-node $NUM_GPUS \
                ...
                ./scripts/translation_FPDM_dilated_visualization.py --name brats $MODEL_FLAGS $DIFFUSION_FLAGS $DIR_FLAGS $DATA_FLAGS $ABLATION_FLAGS $DUAL_THRESHOLD_FLAGS $SNR_WEIGHTING_FLAGS $VISUALIZATION_FLAGS
    done
done
