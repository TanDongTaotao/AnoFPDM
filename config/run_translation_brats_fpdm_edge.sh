...
do
    ...
    do
        # 设置日志输出目录
        export OPENAI_LOGDIR="${LOG_ROOT}/logs_brats_aba/translation_fpdm_edge_${w}_${model_num}_${forward_steps}_${round}_x1"
        # 确保目录存在
        mkdir -p $OPENAI_LOGDIR
        echo "日志将保存在: $OPENAI_LOGDIR"
 
        # 设置数据目录和预训练模型目录
        data_dir="data" # 假设数据在 DATA_ROOT 下
       ...
       ABLATION_FLAGS="--last_only False --subset_interval -1 --t_e_ratio 1 --use_gradient_sam False --use_gradient_para_sam False --use_timestep_weights False --edge_loss True"
        ...
        torchrun --nproc-per-node $NUM_GPUS \
                ...
                ./scripts/translation_FPDM_edge_regularization.py --name brats $MODEL_FLAGS $DIFFUSION_FLAGS $DIR_FLAGS $DATA_FLAGS $ABLATION_FLAGS
    done
done
