...
# 此脚本已修改为可在本地计算机上运行HAE UNet V2 Conservative推理，而非 SLURM 集群。
...
export MASTER_PORT=12364 # 使用一个不同的端口以避免冲突
...
model_num=210000
version=hae_v2_conservative  # Use HAE UNet V2 Conservative version
...
do
    ...
    do
        # 设置日志输出目录
        export OPENAI_LOGDIR="${LOG_ROOT}/logs_brats_aba_hae_v2_conservative/translation_fpdm_hae_v2_conservative_${w}_${model_num}_${forward_steps}_${round}_x1"
        # 确保目录存在
        ...
        model_dir="${LOG_ROOT}/logs_brats_normal_99_11_128_hae_v2_conservative/logs_guided_${threshold}_all_00_${version}_128_norm"
        image_dir="$OPENAI_LOGDIR"

        # 检查模型目录是否存在
        if [ ! -d "$model_dir" ]; then
            echo "错误: 预训练HAE V2 Conservative模型目录不存在: $model_dir"
            echo "请确保您已经训练了HAE V2 Conservative引导模型，或者修改 model_dir 变量指向正确的路径。"
            exit 1
        fi
        ...
        DATA_FLAGS="--batch_size 20 --num_batches 1 \
                    --batch_size_val 20 --num_batches_val 10\
                    --modality 0 3 --use_weighted_sampler False --seed $seed"  # Increased batch size due to memory optimization
        ...
        # --- [步骤 5] ---
        # 运行HAE UNet V2 Conservative图像翻译脚本
        NUM_GPUS=1 # 设置使用的 GPU 数量
        torchrun --nproc-per-node $NUM_GPUS \
                    --nnodes=1\
                    --rdzv-backend=c10d\
                    --rdzv-endpoint=$MASTER_ADDR:$MASTER_PORT\
                ./scripts/translation_FPDM_hae_v2_conservative.py --name brats $MODEL_FLAGS $DIFFUSION_FLAGS $DIR_FLAGS $DATA_FLAGS $ABLATION_FLAGS
    done
done

echo "所有 HAE UNet V2 Conservative 推理任务完成！"