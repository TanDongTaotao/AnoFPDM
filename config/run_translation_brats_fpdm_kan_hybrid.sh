...
model_num=210000
version=kan_hybrid  # Use KAN-UNet hybrid
kan_hidden_ratio=2.0  # KAN hidden layer ratio
...
do
    ...
    do
        # 设置日志输出目录
        export OPENAI_LOGDIR="${LOG_ROOT}/logs_brats_aba/translation_fpdm_kan_hybrid_${w}_${model_num}_${forward_steps}_${round}_x1"
        # 确保目录存在
        ...
        model_dir="${LOG_ROOT}/logs_brats_normal_99_11_128/logs_guided_${threshold}_all_00_kan_hybrid_128_norm"  # Use KAN hybrid model
        image_dir="$OPENAI_LOGDIR"

        # 检查模型目录是否存在
        if [ ! -d "$model_dir" ]; then
            echo "错误: 预训练模型目录不存在: $model_dir"
            echo "请确保您已经训练了引导模型，或者修改 model_dir 变量指向正确的路径。"
            exit 1
        fi
        ...
        torchrun --nproc-per-node $NUM_GPUS \
                ...
                ./scripts/translation_FPDM_kan_hybrid.py --name brats $MODEL_FLAGS $DIFFUSION_FLAGS $DIR_FLAGS $DATA_FLAGS $ABLATION_FLAGS $DUAL_THRESHOLD_FLAGS $SNR_WEIGHTING_FLAGS
    done
done
