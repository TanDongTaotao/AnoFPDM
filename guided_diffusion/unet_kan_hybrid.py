...
class KANLinear(nn.Module):
    def __init__(
        ...
        grid = (
            (
                th.arange(-spline_order, grid_size + spline_order + 1) * h
                + grid_range[0]
            )
        ...
        self.base_weight = nn.Parameter(th.Tensor(out_features, in_features))
        self.spline_weight = nn.Parameter(
            th.Tensor(out_features, in_features, grid_size + spline_order)
        )
    ...
    def reset_parameters(self):
        ...
        with th.no_grad():
            noise = (
                (
                    th.rand(self.grid_size + 1, self.in_features, self.out_features)
                    - 1 / 2
                )
                * self.scale_noise
                / self.grid_size
    ...
    def b_splines(self, x: th.Tensor):
        """
        Compute the B-spline bases for the given input tensor.
        """
        assert x.dim() == 2 and x.size(1) == self.in_features

        grid: th.Tensor = self.grid
        x = x.unsqueeze(-1)
    ...
    def curve2coeff(self, x: th.Tensor, y: th.Tensor):
        """
        ...
        B = y.transpose(0, 1)
        solution = th.linalg.lstsq(A, B).solution
...
class KANBlock(nn.Module):
    """
    KAN block for feature transformation in bottleneck layers
    """
    def __init__(self, in_features, hidden_features=None, out_features=None):
        ...
        self.fc1 = KANLinear(
            in_features,
            out_features,  # 使用 out_features 而不是 hidden_features
            grid_size=grid_size,
    ...
    def forward(self, x, H, W):
        ...
        # fc1 outputs hidden_features, which should be the same as C for this implementation
        x = x.reshape(B, N, C).contiguous()
        return x

# ==================== Original UNet Components ====================
...
class KANResBlock(TimestepBlock):
    """
    KAN-enhanced ResBlock for bottleneck layers.
    Integrates KAN transformation while maintaining ResBlock interface.
    """
    ...
    def __init__(
        ...
        # KAN integration
        self.norm_kan = normalization(self.out_channels)
        self.kan_block = KANBlock(
            in_features=self.out_channels,
            hidden_features=int(self.out_channels * kan_hidden_ratio),
    ...
    def forward(self, x, emb):
        """
        Apply the KAN-enhanced block to a Tensor, conditioned on a timestep embedding.
        """
        return checkpoint(
            self._forward, (x, emb), self.parameters(), self.use_checkpoint
        )
...
class DilatedResBlock(TimestepBlock):
    """
    A residual block with parallel dilated convolutions for enhanced receptive field.
    """
    ...
    def __init__(
        ...
        for dilation in dilation_rates:
            self.dilated_convs.append(
                conv_nd(dims, self.out_channels, self.out_channels // len(dilation_rates), 
                       3, padding=dilation, dilation=dilation)
            )

        # Fusion layer
        self.fusion_conv = conv_nd(dims, self.out_channels, self.out_channels, 1)
        self.fusion_norm = normalization(self.out_channels)
...
class UNetKANHybridModel(nn.Module):
    """
    The full UNet model with KAN integration in bottleneck layers.
    KAN modules are applied to:
    1. Middle block first ResBlock -> KANResBlock
    2. Last encoder layer ResBlock -> KANResBlock
    """
    ...
    def __init__(
        ...
        # Track the last encoder layer for KAN integration
        last_encoder_level = len(channel_mult) - 1
        
        for level, mult in enumerate(channel_mult):
            for i in range(num_res_blocks):
                # Use KANResBlock for the last encoder layer
                if level == last_encoder_level and i == num_res_blocks - 1:
                    layers = [
                        KANResBlock(
                            ch,
                            time_embed_dim,
        ...
        # Modified middle_block with KAN integration
        self.middle_block = TimestepEmbedSequential(
            KANResBlock(  # Replace first ResBlock with KANResBlock
                ch,
                time_embed_dim,
                dropout,
                dims=dims,
...
class EncoderUNetKANHybridModel(nn.Module):
    """
    The half UNet model with KAN integration for encoding to a latent space.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Track the last encoder level for KAN integration
        last_encoder_level = len(channel_mult) - 1
...
# Add missing Timestep class for compatibility
