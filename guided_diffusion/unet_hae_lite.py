...
class AttentionBlock(nn.Module):
    ...
    https://github.com/hojonathanho/diffusion/blob/1e0dceb3b3495bbe19116dcc91b1c5b68b7b8e40/diffusion_tf/models/unet.py#L66.
    """
    ...
    def forward(self, x, cemb_mm=None):
        return checkpoint(
            self._forward, (x, cemb_mm), self.parameters(), True
        )
...
# 新增：多尺度稀疏Transformer块（MSTB）- 精简版
class MultiScaleSparseTransformerBlockLite(nn.Module):
    """
    Multi-Scale Sparse Transformer Block (MSTB) - Lite Version
    借鉴ViT的方式，使用共享的卷积投影层来减少参数量
    """
...
# 新增：混合CNN-Transformer块 - 精简版
class HybridCNNTransformerBlockLite(TimestepBlock):
    """
    混合CNN-Transformer块，结合CNN的局部建模和Transformer的长距离依赖建模 - 精简版
    """
    ...
    def __init__(self, channels, emb_channels, dropout=0.1, use_checkpoint=False):
        ...
        # Transformer分支（使用精简版MSTB）
        self.transformer_branch = MultiScaleSparseTransformerBlockLite(
            channels=channels,
            dropout=dropout
        )
...
class HAEUNetModelLite(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    HAE (Heterogeneous Auto-Encoder) Lite Version - 精简版，使用共享投影层
